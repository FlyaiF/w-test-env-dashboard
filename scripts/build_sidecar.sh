...
if [ "$1" = "all" ]; then
    ...
    GOOS=darwin GOARCH=amd64 go build -ldflags="-s -w" -o "$BUILD_DIR/darwin-amd64/go_sidecar" .
    echo "  darwin/amd64 done"

    GOOS=darwin GOARCH=arm64 go build -ldflags="-s -w" -o "$BUILD_DIR/darwin-arm64/go_sidecar" .
    echo "  darwin/arm64 done"

    GOOS=windows GOARCH=amd64 go build -ldflags="-s -w" -o "$BUILD_DIR/windows-amd64/go_sidecar.exe" .
    echo "  windows/amd64 done"

    GOOS=linux GOARCH=amd64 go build -ldflags="-s -w" -o "$BUILD_DIR/linux-amd64/go_sidecar" .
    echo "  linux/amd64 done"
else
    echo "Building for current platform..."
    go build -ldflags="-s -w" -o "$BUILD_DIR/go_sidecar" .
